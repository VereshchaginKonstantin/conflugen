...
func uploadMermaidDiagrams(
	...
	for _, d := range diagrams {
		...
		// Рендерим SVG через beautiful-mermaid (node, без Chromium)
...
// uploadImageAttachments читает локальные файлы изображений с диска и
// загружает их как вложения к странице. Бинарные данные передаём как строку
// — `buildMultipartBody` пишет байты через `io.WriteString`, который для
// `multipart.Part` (io.Writer) безопасен для произвольного byte-content.
func uploadImageAttachments(
	rawAPI rawRequester,
	baseURL string,
	pageID string,
	images []extensions.ImageAttachment,
) error {
	for _, img := range images {
		data, err := os.ReadFile(img.Path)
		if err != nil {
			log.Printf("предупреждение: не удалось прочитать изображение %s: %v — пропускаю", img.Path, err)
			continue
		}
		if err := uploadAttachment(rawAPI, baseURL, pageID, img.Filename, string(data)); err != nil {
			return fmt.Errorf("upload image %s: %w", img.Filename, err)
		}
		log.Printf("  загружено изображение: %s", img.Filename)
	}
	return nil
}
