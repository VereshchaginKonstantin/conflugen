...
func (t *SpoilerASTTransformer) collectDetailsPairs(
	...
	var current *detailsPair

	// Ходим ТОЛЬКО по top-level siblings документа — иначе ast.Walk проваливается
	// внутрь параграфов/списков и собирает Text/ListItem-ноды как «детей»
	// спойлера; затем reparent ломает структуру (пустой <p />, list items
	// вываливаются из <ul>, теряются <strong>/<em>).
	for n := node.FirstChild(); n != nil; n = n.NextSibling() {
		if htmlBlock, ok := n.(*ast.HTMLBlock); ok {
			content := getNodeContent(htmlBlock, reader)
			if current == nil && strings.Contains(content, "<details") {
				current = &detailsPair{
					openNode: htmlBlock,
					content:  content,
					children: []ast.Node{},
			...
			if current != nil && strings.Contains(content, "</details>") {
				current.closeNode = htmlBlock
				current.closeContent = content
				current = nil
				continue
			}
		}
